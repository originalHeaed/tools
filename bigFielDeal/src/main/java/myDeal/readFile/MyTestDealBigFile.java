...
public class MyTestDealBigFile {
     ...
     *  处理效率高很多：每行记录对外 IO 时间占比越高，使用线程池的效率越高，如果每行记录的处理不需要对外进行 IO 则效率还不如正常模式
     *      （因为出去对外 IO 其他的所有操作都需要一直使用 CPU，通过并发获取的效率提高不大）
     * 使用正常按行处理文件：
     *  内层占用低：每次只读取一行（当然也可以批量读取，但总体上比使用线程池的低）
     *  处理效率低：串行处理，如果每行记录处理过程中对外 IO 操作多，由于但线程需要等待 IO 完成，因此 CPU 利用率低，效率低
     *
     *  自测1：
     ...
     *  普通方法：总耗时 122s
     *
     *  总结：内存占用基本都没啥大问题，每行记录对外 IO 处理时间占高建议使用线程池，占比低建议使用正常模式（减少线程之间的切换，反而还提高了效率）
     */
